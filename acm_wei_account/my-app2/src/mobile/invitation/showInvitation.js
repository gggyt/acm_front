...
class AllInvitationView extends React.Component {
  ...
  render() {
    return (
      <div>
      <div>
        <Affix offsetTop={this.state.height-70} onChange={affixed => console.log(affixed)}>
        <Link to="/mobile/addInvitation">
        <Button size="large"  style={{float:'right'}} type="primary" shape="circle">
        <Icon type="plus" />
        </Button>
        </Link>
      </Affix>
      ...
      <div>
      <span>&nbsp;&nbsp;&nbsp;<Link to = "/mobileIndex">首页</Link>>论坛</span>
                  ...
                  <WingBlank size="lg">
                    ...
                    <Card>
                      <Card.Header
                        ...
                        thumb={
                          item.imageUrl==''?<Avatar shape="square" size={30} style={{ backgroundColor: '#f56a00', verticalAlign: 'middle' }} >
                            {item.createUser}
                          </Avatar>:
                        item.imageUrl
                        }
